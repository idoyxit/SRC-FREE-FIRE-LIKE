...
KEY_LIMIT = 99999999999999999999999          # ← change to e.g. 500 if you want more likes per IP per day
...
def handle_requests():
    ...
    valid_servers = ["IND", "BR", "US", "SAC", "NA", "ID", "RU", "BD"]
...
def add_account():
    ...
    Usage: /add_account?uid=123456&pass=xyz&region=IND&key=IDOY
    """
    ...
    valid_regions = ["IND", "BR", "US", "SAC", "NA", "ID", "RU", "BD"]
...
def reset_cache():
    ...
    if key != "IDOY":
        return jsonify({"error": "Invalid key"}), 403
    liked_cache.clear()
    return jsonify({"message": "Cache cleared", "credit": "@IDOYXITERZZ"})

if __name__ == '__main__':
    print("🚀 Smart Like API with Account Manager")
    print("📍 Endpoints:")
    print("   GET  /like?uid=UID&server_name=REGION&key=IDOY")
    print("   GET  /add_account?uid=...&pass=...&region=...&key=IDOY")
    print("   GET  /reset-cache?key=IDOY")
